...

{
...

    {
        ...
        if (!empty($arg['packages'])) {

            $arg['packages'] = array_values($arg['packages']);
            // dd($arg['packages']);
        }
...

    {
        ...
        // $packages_input = $request->input('packages');
        ...
        if ($meta) {
            ...
            $meta->packages = $dataPriceAvailability['packages'] ?? $meta->packages;

            if ($meta->enable_packages and !empty($meta->packages)) {

                if (!empty($meta->packages)) {

                    foreach ($meta->packages as $k => $type) {

                        if (isset($packages_input[$k]) and $packages_input[$k]['enable']) {
        ...
        if ($check) {
            ...
            $booking->addMeta('packages', $packages);
...

    {
        ...
        if (!empty($datesRaw)) {
            ...
            $dates = [
                ...
                'packages' => is_array($datesRaw->packages) ? $datesRaw->packages : false,
...

    {
        ...
        if ($meta) {
            ...
            if ($meta->enable_packages) {

                $booking_data['packages'] = $meta->packages;

                if (!empty($booking_data['packages'])) {

                    foreach ($booking_data['packages'] as $k => &$type) {

                        if (!empty($lang)) {

                            $type['name'] = !empty($type['name_' . $lang])?$type['name_' . $lang]:$type['name'];

                            $type['description'] = !empty($type['description' . $lang])?$type['description' . $lang]:$type['description'];

                        }

                        $type['price'] = (int)$type['price'];

                        $type['start_times'] = array_values((array)$type['start_times']);
                    ...
                    $booking_data['packages'] = array_values((array)$booking_data['packages']);

                } else {

                    $booking_data['packages'] = [];
     ...
     * This function edited by LAYERS
    ...
    public function isFormEnquiryAndBook()

    {
        ...
        $customTourIds = setting_item('custom_tours_enquiry_ids');

        $customTourIdsArray = explode(',', $customTourIds);



        if (in_array($this->id, $customTourIdsArray)) {
    ...
    public function getBookingEnquiryType()

    {
        ...
        if (in_array($tour_id, $customTourIdsArray)) {
